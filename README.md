# [ng-module-boilerplate](https://github.com/PSDCoder/pg.ng-module-boilerplate)
***

Hello. It's just a boilerplate for simplify development modules for angular.js
...
```sh
$ git clone git://github.com/PSDCoder/pg.ng-module-boilerplate you-module-name
$ cd you-module-name
...
* **For using csscomb auto aligner**: 
    * `cat pre-commit-hook.sh > .git/hooks/pre-commit`
    * Add executable permissions `chmod ug+x .git/hooks/pre-commit`

###List of common tasks:

* `gulp [default] [--not-open]` - run default server with watchers and open it in browser.
* `NODE_ENV=prod gulp [--not-open]` - build production version and run server for see result. Watchers doesn't work.
* `gulp test` - run all tests.
* `gulp build` - build production version.
* `gulp csscomb` - manually start CSSComb clean up hook.
* `gulp release --major|--minor|--patch|--release [--not-push]` - Release new version of app.  
It's changed version of app in package.json, commit that change, added tag and push it to **origin/master**  
